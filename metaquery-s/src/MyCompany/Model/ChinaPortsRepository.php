 ...
 */
class ChinaPortsRepository
{

    protected $ChinaPortsTable;

    public function __construct()
    {
        $this->ChinaPortsTable = new TableGateway(
            'm_china_port',
            new Adapter(array(
                'driver' => 'Pdo_Sqlite',
    ...
    {
        $inserted = $this->ChinaPortsTable->insert(array(
            'name' => $name,
            'location' => $location,
            'activity' => $activity,
            'phone' => $phone
        ...
        if($inserted)
            return $this->ChinaPortsTable->getLastInsertValue();
        return -1;
    }

    public function getAll()
    {
        return $this->ChinaPortsTable->select()->toArray();
    }

    public function get($id)
    {
        $resultSet = $this->ChinaPortsTable->select(array('id = ?' => $id));
    ...
    {
        return (bool) $this->ChinaPortsTable->update(
            array(
                'name' => $name,
                'location' => $location,
                'activity' => $activity,
    ...
    {
        return $this->ChinaPortsTable->delete(array('id = ?' => $id));
    }
